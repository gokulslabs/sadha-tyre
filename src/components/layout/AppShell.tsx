    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,oklch(0.98_0.008_255)_0%,var(--background)_32%)]">
      <header className="sticky top-0 z-30 flex h-[68px] items-center gap-3 border-b border-border/70 bg-card/95 px-4 shadow-sm backdrop-blur md:px-6">
        ...
        <div className="flex items-center gap-2 border-r border-border pr-4">
          <span className="grid h-8 w-8 place-items-center rounded-lg bg-violet-600 text-white"><Truck className="h-4 w-4" /></span>
        ...
        <div className="ml-auto flex items-center gap-2">
          <div className="hidden items-center gap-2 rounded-md border border-border bg-background px-3 py-2 text-sm font-semibold sm:flex"><WalletCards className="h-4 w-4 text-violet-600" /> ₹0.00</div>
          <Button size="sm" className="hidden bg-violet-600 hover:bg-violet-700 sm:inline-flex"><Truck className="h-4 w-4" /> Add Trip</Button>
      ...
      {sidebarOpen && (
        <>
          ...
          <aside className="fixed inset-y-0 left-0 z-50 w-[280px] border-r border-border bg-card p-5 shadow-xl">
            <div className="mb-7 flex items-center justify-between"><div className="flex items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-lg bg-violet-600 text-white"><Truck className="h-4 w-4" /></span><span className="text-sm font-semibold">SADHA TYRE</span></div><button type="button" aria-label="Close Sidebar Menu" className="rounded-md p-2 text-muted-foreground hover:bg-muted" onClick={() => setSidebarOpen(false)}>×</button></div>
            <p className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Maintenance</p>
            <nav className="space-y-1"><a href="#tyre-view" onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 rounded-md bg-violet-50 px-3 py-2.5 text-sm font-medium text-violet-700"><Truck className="h-4 w-4" /> Tyre View</a><a href="#inventory" onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 rounded-md px-3 py-2.5 text-sm text-muted-foreground hover:bg-muted"><WalletCards className="h-4 w-4" /> Tyre Inventory</a><a href="#history" onClick={() => setSidebarOpen(false)} className="flex items-center gap-3 rounded-md px-3 py-2.5 text-sm text-muted-foreground hover:bg-muted"><Bell className="h-4 w-4" /> Audit Log</a></nav>
          </aside>
        </>
      )}
      ...
      <button type="button" aria-label="Toggle support chat" onClick={() => setChatOpen((v) => !v)} className="fixed bottom-5 right-5 z-40 grid h-14 w-14 place-items-center rounded-full bg-violet-600 text-white shadow-lg transition hover:bg-violet-700"><MessageCircle className="h-6 w-6" /></button>
    </main>
  );
}
