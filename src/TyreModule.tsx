...
function TyreViewSection() {
  ...
  return <div className="space-y-4">
    ...
    {selected && <div role="dialog" aria-label={`Tyre ${selected.position_code} details`} className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/30 p-4 backdrop-blur-[2px]"><div className="w-full max-w-xl rounded-xl border border-border bg-card p-6 shadow-lift"><div className="mb-5 flex items-start justify-between"><div><p className="text-xs font-medium uppercase tracking-widest text-muted-foreground">Tyre position</p><h3 className="mt-1 text-xl font-semibold">{selected.position_code}</h3><p className="mt-1 text-sm text-muted-foreground">{selected.axle_label || "Truck tyre"}</p></div><Button variant="outline" onClick={() => setSelected(null)}>Close</Button></div>{!editingDetails ? <><div className="mb-5 rounded-lg bg-destructive/10 p-4"><p className="text-sm font-semibold text-destructive">{tyreHealth(Number(selected.current_km)) === "Replace" ? "Replace Soon" : tyreHealth(Number(selected.current_km))}</p><p className="mt-1 text-xs text-muted-foreground">Tyre type: {selected.tyre_type}</p></div><div className="grid gap-3 sm:grid-cols-2"><StatCell label="Installed on" value={selected.fitted_on || "—"} /><StatCell label="Current KM" value={Number(selected.current_km).toLocaleString("en-IN")} /><StatCell label="Tyre usage" value={`${Number(selected.current_km).toLocaleString("en-IN")} km`} /><StatCell label="Installation cost" value={inr(Number(selected.cost))} /><StatCell label="Brand" value={selected.brand || "—"} /><StatCell label="Part / Serial number" value={selected.serial_no || "—"} /></div><div className="mt-6 flex justify-end gap-2"><Button variant="outline" onClick={() => toast.info("Replacement workflow will be recorded in Tyre Events.")}>Replace Tyre</Button><Button onClick={() => setEditingDetails(true)}>Edit Details</Button></div></> : <><div className="grid gap-3 sm:grid-cols-2"><Field label="Brand"><Input value={draft.brand} onChange={(e) => setDraft((d) => ({ ...d, brand: e.target.value }))} /></Field><Field label="Serial No"><Input value={draft.serial_no} onChange={(e) => setDraft((d) => ({ ...d, serial_no: e.target.value }))} /></Field><Field label="Current KM"><Input type="number" value={draft.current_km} onChange={(e) => setDraft((d) => ({ ...d, current_km: e.target.value }))} /></Field><Field label="Cost"><Input type="number" value={draft.cost} onChange={(e) => setDraft((d) => ({ ...d, cost: e.target.value }))} /></Field><Field label="Remark"><Input value={draft.remark} onChange={(e) => setDraft((d) => ({ ...d, remark: e.target.value }))} /></Field></div><div className="mt-6 flex justify-end gap-2"><Button variant="outline" onClick={() => setEditingDetails(false)}>Cancel</Button><Button onClick={saveSelected} disabled={save.isPending}>Save Tyre</Button></div></>}</div></div>}
  </div>;
}
