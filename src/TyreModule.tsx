...
function TyreViewSection() {
          ...
          <div key={i} className="flex justify-center gap-2">
            {row.map((t) => {
              ...
              return <button key={t.id} type="button" disabled={t.id.startsWith("preview-")} onClick={() => selectTyre(t)} aria-label={`Tyre ${t.position_code}`} className={`relative h-[76px] w-[76px] rounded-full bg-card ring-2 transition hover:scale-105 ${cls.ring} ${selected?.id === t.id ? "ring-4 ring-primary" : ""} ${t.id.startsWith("preview-") ? "cursor-default" : ""}`}>
                ...
                <span className="absolute inset-x-0 bottom-2 text-[9px] uppercase text-muted-foreground">{t.tyre_type}</span>
              </button>;
            })}
          </div>
        ))}
  ...
  return <div className="space-y-4">
    ...
    <div className="rounded-lg border border-border bg-card p-4 shadow-panel"><div className="grid min-w-[900px] grid-cols-[1fr_280px_1fr] items-center gap-5 overflow-x-auto"><div className="space-y-3">{leftAxles.map(([label, list]) => axleBlock(label, list))}</div><div className="flex min-h-[220px] flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-border bg-muted/20 p-5"><Truck className="h-10 w-10 text-muted-foreground" /><p className="text-center text-xl font-bold tracking-[0.2em]">{showAddVehicle ? (newVehicle.vehicle_number || "NEW VEHICLE") : vehicle.vehicle_number}</p><div className="grid w-full grid-cols-2 gap-3 border-t border-border pt-3 text-center text-xs"><div><span className="text-muted-foreground">AXLES</span><br /><strong className="text-xl">{(showAddVehicle ? axlePlan(Number(newVehicle.wheels)) : plan).steer + (showAddVehicle ? axlePlan(Number(newVehicle.wheels)) : plan).rear}</strong></div><div><span className="text-muted-foreground">WHEELS</span><br /><strong className="text-xl">{showAddVehicle ? newVehicle.wheels : vehicle.wheels}</strong></div></div></div><div className="space-y-3">{rightAxles.map(([label, list]) => axleBlock(label, list))}</div></div><p className="mt-4 text-center text-xs text-muted-foreground">{showAddVehicle ? "Preview of the selected wheel configuration — create the vehicle to save it" : "Click on any tyre to view detailed information"}</p></div>
