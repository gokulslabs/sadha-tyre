...
function TyreViewSection() {
        ...
        {[list.filter((t) => /R/.test(t.position_code.replace(/^\d+/, ""))), list.filter((t) => /L/.test(t.position_code.replace(/^\d+/, "")))].map((row, i) => (
          <div key={i} className="flex justify-center gap-2">
            {row.map((t) => {
              ...
              return <button key={t.id} type="button" disabled={t.id.startsWith("preview-")} onClick={() => selectTyre(t)} aria-label={`Tyre ${t.position_code}`} className={`relative h-[104px] w-[104px] rounded-full bg-card transition hover:scale-105 ${hasData ? `ring-2 ${cls.ring}` : "border-2 border-dashed border-muted-foreground/20"} ${selected?.id === t.id ? "ring-4 ring-primary" : ""} ${t.id.startsWith("preview-") ? "cursor-default" : ""}`}>
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-border/70 bg-card px-5 py-4 shadow-panel">
      <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary text-primary-foreground shadow-sm"><Truck className="h-5 w-5" /></div><div><h2 className="text-[22px] font-semibold tracking-[-0.02em]">Truck Tyre View</h2><p className="mt-0.5 text-xs text-muted-foreground">Inspect every wheel position, usage, and replacement history.</p></div></div>
    ...
    <div className="overflow-x-auto rounded-lg border border-border bg-card p-4 shadow-panel"><div className="grid min-w-[1120px] grid-cols-[150px_300px_1fr] items-center gap-4"><div className="space-y-3">{leftAxles.map(([label, list]) => axleBlock(label, list))}</div><div className="flex min-h-[200px] flex-col items-center justify-center gap-2 rounded-md border-2 border-dashed border-border bg-gradient-to-r from-muted/50 via-muted/80 to-muted/50 p-4"><Truck className="h-9 w-9 text-muted-foreground" /><p className="text-center text-xl font-bold tracking-[0.2em]" style={{ writingMode: "vertical-rl" }}>{showAddVehicle ? (newVehicle.vehicle_number || "NEW VEHICLE") : vehicle.vehicle_number}</p><div className="grid w-full grid-cols-2 gap-3 border-t border-border pt-2 text-center text-xs"><div><span className="text-muted-foreground">AXLES</span><br /><strong className="text-xl">{(showAddVehicle ? axlePlan(Number(newVehicle.wheels)) : plan).steer + (showAddVehicle ? axlePlan(Number(newVehicle.wheels)) : plan).rear}</strong></div><div><span className="text-muted-foreground">READING</span><br /><strong className="text-base">{showAddVehicle ? "0 km" : `${Number(vehicle.odometer).toLocaleString("en-IN")} km`}</strong></div></div></div><div className="grid grid-cols-3 gap-3">{rightAxles.map(([label, list]) => axleBlock(label, list))}</div></div><p className="mt-4 text-center text-xs text-muted-foreground">{showAddVehicle ? "Preview of the selected wheel configuration — create the vehicle to save it" : "Click on any tyre to view detailed information"}</p></div>
