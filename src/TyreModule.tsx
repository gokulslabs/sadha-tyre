...
function TyreViewSection() {
        ...
        {[list.filter((t) => /R/.test(t.position_code.replace(/^\d+/, ""))), list.filter((t) => /L/.test(t.position_code.replace(/^\d+/, "")))].map((row, i) => (
          <div key={i} className="flex justify-center gap-2">
            {row.map((t) => {
              ...
              return <button key={t.id} type="button" disabled={t.id.startsWith("preview-")} onClick={() => selectTyre(t)} aria-label={`Tyre ${t.position_code}`} className={`relative h-[86px] w-[86px] rounded-full bg-card transition hover:scale-105 ${hasData ? `ring-2 ${cls.ring}` : "border-2 border-dashed border-muted-foreground/20"} ${selected?.id === t.id ? "ring-4 ring-primary" : ""} ${t.id.startsWith("preview-") ? "cursor-default" : ""}`}>
  ...
  return <div className="space-y-4">
    ...
    <div className="overflow-x-auto rounded-lg border border-border bg-card p-4 shadow-panel"><div className="grid min-w-[1040px] grid-cols-[170px_360px_1fr] items-center gap-5"><div className="space-y-3">{leftAxles.map(([label, list]) => axleBlock(label, list))}</div><div className="flex min-h-[240px] flex-col items-center justify-center gap-3 rounded-md border-2 border-dashed border-border bg-gradient-to-r from-muted/40 via-muted/70 to-muted/40 p-5"><Truck className="h-10 w-10 text-muted-foreground" /><p className="text-center text-xl font-bold tracking-[0.2em]" style={{ writingMode: "vertical-rl" }}>{showAddVehicle ? (newVehicle.vehicle_number || "NEW VEHICLE") : vehicle.vehicle_number}</p><div className="grid w-full grid-cols-2 gap-3 border-t border-border pt-3 text-center text-xs"><div><span className="text-muted-foreground">AXLES</span><br /><strong className="text-xl">{(showAddVehicle ? axlePlan(Number(newVehicle.wheels)) : plan).steer + (showAddVehicle ? axlePlan(Number(newVehicle.wheels)) : plan).rear}</strong></div><div><span className="text-muted-foreground">READING</span><br /><strong className="text-base">{showAddVehicle ? "0 km" : `${Number(vehicle.odometer).toLocaleString("en-IN")} km`}</strong></div></div></div><div className="grid grid-cols-3 gap-5">{rightAxles.map(([label, list]) => axleBlock(label, list))}</div></div><p className="mt-4 text-center text-xs text-muted-foreground">{showAddVehicle ? "Preview of the selected wheel configuration — create the vehicle to save it" : "Click on any tyre to view detailed information"}</p></div>
